...
function getDaily(res) {
	...
	$(".daily").find(".time").html(dspDate(time, 6));
	$(".daily").find(".city").find("span").html(city);
	$(".daily").find(".icon").attr("style", 'background:url('+path+res.weather[0].icon+'.svg) no-repeat center top;');
	$(".daily").find(".temp").find("span").eq(0).html(res.main.temp);
	$(".daily").find(".temp-sub").find("span").eq(0).html(res.main.temp_min);
	$(".daily").find(".temp-sub").find("span").eq(1).html(res.main.temp_max);
...
function getWeekly(res) {
	...
	for(var i in res.list) {
		...
		html  = '<ul>';
		html += '<li style="height: 80px;"><img src="'+path+w.icon+'.png'+'" class="img mt-4"></li>';
		html += '<li>';
		html += '<div class="date">'+dspDate(kts, 7).substring()+'</div>';
		html += '<div class="temp"><span>'+m.temp+'º</span> ('+m.temp_min+'º - '+m.temp_max+'º)</div>';
